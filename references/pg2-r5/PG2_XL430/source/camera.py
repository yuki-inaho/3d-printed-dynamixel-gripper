"""Optional, fixed 65-degree camera saddle. Generic 32 mm board, 28 mm hole grid.
...
TILT=65
ORIGIN=(0,56,20)
def cam_pose(s):return s.rotate((0,0,0),(1,0,0),TILT).translate(ORIGIN)
def plate():
    ...
    for x in [-19,19]:s=cut(s,cyl(1.7,3.4,x,0,-3.2),nut_pocket(x,0,-2.6,2.7))
    return s

def stand():
    foot=fuse(box(-12,12,34.5,47,12.4,15.4),box(-29,29,42,47,12.4,15.4))
    for x in [-8,8]:foot=cut(foot,cyl(1.7,4,x,38.5,12.2))
    prof=[(42,12.4),(47,15.4),(66,23),(69,14),(56,9),(44,8)] # Y,Z
    # extrude shape along X
    wire=cq.Wire.makePolygon([cq.Vector(25,y,z) for y,z in prof]+[cq.Vector(25,*prof[0])])
    arm=cq.Solid.extrudeLinear(wire,[],cq.Vector(4,0,0))
    ...
    for side in [-1,1]:
        lug=box(15,29,-5,5,-10,-3)
        if side<0:lug=lug.mirror('YZ')
        lug=cut(lug,cyl(1.7,7.4,side*19,0,-10.2))
...
def optional_parts():
    ...
    for x in [-19,19]:
        put(f'camera_plate_M3_{x}',cam_pose(bolt(3,12,0,0,0,5.5,3).rotate((0,0,0),(1,0,0),180).translate((x,0,-10))),(.66,.68,.72),'fastener','steel')
        put(f'camera_plate_nut_{x}',cam_pose(hex_nut(x,0,-2.4)),(.64,.66,.69),'fastener','steel')
