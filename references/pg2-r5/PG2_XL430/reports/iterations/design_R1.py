...
class Parameters:
    ...
    close_angle:float=150.0
    closed_gap:float=0.8
    face_width:float=132.0
    face_height:float=84.0
    jaw_width:float=24.0
    jaw_y:float=36.0
    jaw_back:float=4.8
    jaw_top:float=9.0
...
def bolt(d,L,x,y,seat,head_d=None,head_h=None,countersunk=False):
    ...
    if countersunk:
        ...
        return fuse(cyl(d/2,L-hd,x,y,seat-L),h)
    return fuse(cyl(d/2,L,x,y,seat-L),cyl(head_d/2,head_h,x,y,seat))
def frame(p=P):
    # Full-width back bearing rails + ends. Central void permits fastener ends and servo horn.
    s=cut(box(-66,66,-42,42,.5,4.5),box(-59,59,-28,28,0,5))
    s=fuse(s,box(-66,66,36.6,42,4.5,9.4),box(-66,66,-42,-36.2,4.5,9.4))
    # End stop fences: bound jaw at approximately 27..153 degrees (not to be driven against).
    s=fuse(s,box(-66,-61.0,-36.2,36.6,4.5,9.4),box(61.0,66,-36.2,36.6,4.5,9.4))
    for y in [-38.5,38.5]:
        for x in [-62,-21,21,62]:
            s=cut(s,cyl(1.7,10,x,y,.3),nut_pocket(x,y,.4,2.7))
    for x in [-23,23]:
        for y in [-32,32]:
...
def retainer():
    s=box(-66,66,32,42,9.4,12.4)
    for x in [-62,-21,21,62]:s=cut(s,cyl(1.7,3.4,x,38.5,9.2))
...
def jaw(p=P):
    ...
    z0=p.jaw_back
    s=fuse(box(-12,12,22,36,z0,9),box(-12,12,-36,-22,z0,9),
           box(5.5,12,-23,23,z0,9),cyl(5.5,9-z0,z=z0),box(0,12,-5.5,5.5,z0,9))
    # Short open window: big pads at front, no slender projecting fingers.
    plate=cut(box(-c,-c+8,-32,32,18,p.finger_tip),box(-c-.1,-c+8.1,-20,20,25,39))
    prof=[(-12,8),(12,8),(12,14),(-c+8,23),(-c,23),(-c,18),(-12,18)]
...
def crank(p=P):
    ...
    for x,y in [(8,0),(-8,0),(0,8),(0,-8)]:
        s=cut(s,cyl(1.15,10,x,y,-.2),cyl(2.15,6.2,x,y,3))
    for x in [-p.r,p.r]:
...
class Model:
    ...
    def at(self,a):
        ...
        for hx,hy in [(8,0),(-8,0),(0,8),(0,-8)]:
            put(f'horn_M2_{hx}_{hy}',pose(bolt(2,6,hx,hy,3,3.8,1.6),a),(.68,.70,.73),'rotor','fastener','steel')
        for side,rotation in [(1,0),(-1,180)]:
            ...
            for kind,px,py,grp in [('crank',side*ax,side*ay,'rotor'),('jaw',side*x,0,f'jaw_{side}')]:
                ...
                b=bolt(3,10,0,0,14,5.5,3)
                n=hex_nut(0,0,4.1 if kind=='crank' else 4.9)
