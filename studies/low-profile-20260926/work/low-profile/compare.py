...
def loading(mount, original, arm, extension):
    ...
    masses = np.array(
        [i["mass_g"] / 1000 for i in items]
        + [0.25]
        + [i["signed_mass_kg"] for i in changes]
    )
    adjusted = {}
...
def main():
    ...
    shortlist = [
        ...
        for r in rows
        if not any(
            np.all(vector(q) <= vector(r)) and np.any(vector(q) < vector(r))
            for q in rows
        )
    ]
    ...
    for row in shortlist:
        record = {
            k: row[k]
            for k in ["id", "pitch", "extension_mm", "pose", "minimum_cube_visible"]
        }
        try:
        ...
        print(
            ...
            round(
                record.get("loads", {})
                .get("camera_group", {})
                .get("height_above_motor_top_mm", 0),
                2,
            ),
            "new hits",
