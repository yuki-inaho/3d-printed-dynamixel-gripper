...
def tool(head, axis, withdrawal=100):
    ...
    shapes = []
    for radius, offset, length in [(1.5, 0.05, 75), (10, 75.05, 30), (40, 70.05, 100)]:
        shapes.append(
            cq.Solid.makeCylinder(
                radius,
                length + withdrawal,
                cq.Vector(*(head + offset * axis)),
                cq.Vector(*axis),
            )
        )
...
def inspect(envelope, parts, target):
    ...
    for name, shape in parts.items():
        gap = 0.01 if name == target else 0.3
...
def main():
    ...
    for i, (x, y, z) in enumerate(ANCHORS):
        tests.append(
            (
                f"CAM5_BASE_TAP_{i}",
                (x, y, TOP + spec.base_seat_thickness + 0.5 + 2),
                (0, 0, 1),
                "base_before_carrier",
            )
        )
    for i, x in enumerate([-10, 10]):
        tests.append(
            (
                f"CAM5_D405_SCREW_{i}",
                r5.local_point(
                    (x, 0, spec.plate_thickness - spec.d405_counterbore_depth + 3), spec
                ),
                norm,
                "camera_carrier_bench",
            )
        )
    for i, x in enumerate([-spec.tie_bolt_local[0], spec.tie_bolt_local[0]]):
        tests.append(
            (
                f"CAM5_TIE_BOLT_{i}",
                r5.local_point(
                    (x, spec.tie_bolt_local[1], spec.plate_thickness + 0.5 + 3), spec
                ),
                norm,
                "final_installed",
            )
    ...
    report = {
        ...
        "assumed_envelope": {
            ...
            "hand_radius_mm": 40,
            "hand_length_mm": 100,
            "positive_axis_withdrawal_mm": 100,
        },
        ...
        "physical_assembly_approved": False,
        "checks": [],
        "base_stack": screw_stack(
            spec.base_screw_length, spec.base_seat_thickness, 0.5, spec.hole_depth
        ),
    ...
    for target, head, axis, stage in tests:
        ...
        if stage == "base_before_carrier":
            present = {
                ...
                if not n.startswith("CAM5_")
                or n == "CAM5_BASE"
                or n.startswith(("CAM5_BASE_TAP_", "CAM5_BASE_WASHER_"))
            }
        ...
        report["checks"].append(
            {
                ...
                "absent_at_stage": sorted(set(parts) - set(present)),
                "installed_nonpass": installed,
                "stage_nonpass": staged,
                "stage_envelope_pass": not staged,
            }
        )
    ...
    report["all_staged_envelopes_pass"] = all(
        r["stage_envelope_pass"] for r in report["checks"]
    )
    report["limitations"] = [
        "Actual tool socket fit and tightening torque unknown",
        "Nut loading/retention and cluster insertion are separate from above-head access",
        "Free cable routing beyond nominal plug and stub unverified",
        "Full physical fastening/printing acceptance remains open",
    ]
    save("service-validation.json", report)
