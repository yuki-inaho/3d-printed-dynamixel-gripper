...
def main():
    ...
    url = (
        f"https://raw.githubusercontent.com/roboninecom/SO-ARM100-101-Parallel-Gripper/{COMMIT}/models/parts/"
        + urllib.parse.quote(refname)
    )
    target = REF / refname
    ...
    report = {
        "inputs": [
            ...
            for p in (SOURCE, COMPACT, target)
        ],
        "reference_url": url,
        "parts": {},
    }
    for row in rows:
        ...
        for face in s.Faces():
            if face.geomType() != "PLANE":
                continue
            report["parts"][row.name]["faces"].append(
                {
                    "area": face.Area(),
    ...
    report["reference_mesh"] = {
        "bounds": mesh.bounds.tolist(),
        "watertight": bool(mesh.is_watertight),
        "principal_planar_normals": [],
    }
    normals = {}
    ...
    report["reference_mesh"]["principal_planar_normals"] = sorted(
        [{"normal": k, "area": a} for k, a in normals.items()], key=lambda x: -x["area"]
    )[:12]
    (OUT / "reports/input-intake.json").write_text(json.dumps(report, indent=2))
    print(
        json.dumps(
            {k: {j: x for j, x in v.items() if j != "faces"} for k, v in report["parts"].items()},
            indent=2,
        )
    )
