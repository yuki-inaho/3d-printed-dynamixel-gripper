...
chosen = min(viable, key=lambda r: (r['pitch'], r['extension_mm'], r['loads']['camera_group']['height_above_motor_top_mm']))
optical = next(r for r in json.loads((OUT/'reports/optical-shortlist.json').read_text()) if r['id'] == chosen['id'])
selection = optical | chosen | {'status':'selected_for_full_validation',
...
for r in data['candidates']:
    m = r['loads']
    rows.append(f"|{r['id']}|{r['pitch']}|{r['extension_mm']}|{m['camera_group']['height_above_motor_top_mm']:.2f}|{m['camera_group']['mass_g']:.2f}|{m['added_finger_pad_mass_g']:.2f}|{m['partial_load_including_finger_pad_delta']['wrist']['reference_abs_Mx_Nm']:.6f}|{'PASS' if r['static_candidate_pass'] else 'FAIL'}|")
m = chosen['loads']
...
rows += ['',f"採用理由: 参考30°に到達する最小延長は比較グリッド内で20mm。その中で高さが最も低い候補を選んだ。高さは{base_h-height:.2f}mm低下。手首の比較部分モーメントは{base_m:.6f}→{moment:.6f}Nm（{(moment/base_m-1)*100:+.2f}%）。",'',
