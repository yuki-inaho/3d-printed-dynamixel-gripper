...
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "work/optimization"))
...
def screen(p, delta):
    ...
    for edge, side in itertools.product([10, 20, 30], ["left", "right"]):
        pts = np.array(
            list(itertools.product([-edge / 2, edge / 2], repeat=3))
        ) + target_center(delta)
...
def search():
    ...
    for extension in range(0, 31, 5):
        rows = []
        for pitch, y, z in itertools.product(
            range(30, 51, 5), range(120, 196, 5), range(205, 246, 5)
        ):
            p = pose([-0.2, y, z], pitch)
            checks = screen(p, extension)
            row = {
                "id": f"E{extension}_P{pitch}_Y{y}_Z{z}",
                "extension_mm": extension,
                "pitch": pitch,
                "pose": p,
                "screen": checks,
                "screen_pass": all(
                    v["inside"] and v["min_depth_mm"] >= 75 for v in checks
                ),
                "bare_optics": [],
                "bare_optics_pass": False,
            }
            rows.append(row)
        ...
        for r in active:
            ...
            r["minimum_cube_visible"] = min(
                v["visible_fraction"] for v in r["bare_optics"]
            )
        all_rows += rows
        save(
            "search.json",
            {
                "grid": all_rows,
                "mount_in_scene": False,
                "resolution": SIZE,
                "rejection_short_circuit": True,
                "finished_extension_mm": extension,
            },
        )
        print(
            "EXTENSION COMPLETE",
