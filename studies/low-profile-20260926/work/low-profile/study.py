...
from paths import OPTIMIZATION
...
def screen(p, delta):
    ...
    for edge, side in itertools.product([10, 20, 30], ["left", "right"]):
        pts = np.array(list(itertools.product([-edge / 2, edge / 2], repeat=3))) + target_center(
            delta
        )
    ...
    for extension in range(0, 31, 5):
        ...
        for pitch, y, z in itertools.product(
            ...
            row = {
                ...
                "pose": p,
                "screen": checks,
                "screen_pass": all(v["inside"] and v["min_depth_mm"] >= 75 for v in checks),
                "bare_optics": [],
