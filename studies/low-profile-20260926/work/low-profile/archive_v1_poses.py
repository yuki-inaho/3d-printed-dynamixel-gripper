...
for f in reports.glob('native-step-*.json'):
    if f.name in ('native-step-geometry-diagnostic.json','native-step-version-yaw10-exported-zero.json'):
        continue
...
for e in data['exports']:
    if e['path'] in mapping:
        ...
        e['reason']='Superseded: PG3_pad_L/R followed opposite jaw in pre-repair assembly'
