...
script.write_text((HERE/'increment-pose.template.js').read_text().replace('__CASE__', json.dumps(spec)))
proc = subprocess.run(['rtk','proxy','npx','--yes','@playwright/cli','-s=onshape-headless','run-code','--filename=low-profile/current-increment.js'],cwd=ROOT/'work',capture_output=True,text=True,timeout=1800)
...
if 'failed_case' in report:
    report.setdefault('failure_history', []).append({'case':report.pop('failed_case'),'error':report.pop('error'),'resolution':'UI incremental pose'})
