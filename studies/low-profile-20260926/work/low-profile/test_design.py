...
def test_independent_boolean_controls_detect_empty_common(monkeypatch):
    original = cq.Solid.makeBox(2, 3, 4)
    real_intersect = cq.Shape.intersect

    def broken_intersect(self, other, *args, **kwargs):
        if self is other:
            return real_intersect(self, other, *args, **kwargs)
        return cq.Compound.makeCompound([])

    monkeypatch.setattr(cq.Shape, "intersect", broken_intersect)
    with pytest.raises(ValueError, match="Independent Boolean intersection"):
        symmetric_difference(original, original.copy())


def test_known_solid_difference_and_root_damage(arm):
    box = cq.Solid.makeBox(2, 3, 4)
    assert symmetric_difference(box, box.copy()) == pytest.approx(0, abs=1e-8)
    assert symmetric_difference(box, box.translate((10, 0, 0))) == pytest.approx(48)
    original = arm["PG3_finger_R"]
    revised = extend_finger(original, 20)
    assert not preservation(original, revised.translate((0.5, 0, 0)), 20)["pass"]
