...
def main():
    ...
    for name,shape in parts.items():
        other = new[name]
        err = {'name':name,'volume_mm3':abs(shape.Volume()-other.Volume()),'area_mm2':abs(shape.Area()-other.Area()),
               'bounds_mm':float(np.abs(np.array(bounds(shape))-bounds(other)).max()),
               'same_solid_count':len(shape.Solids())==len(other.Solids())}
        err['pass'] = err['volume_mm3']<.01 and err['area_mm2']<.01 and err['bounds_mm']<1e-4 and err['same_solid_count']
        checks.append(err)
    ...
    mask_checks = {side:preservation(source[f'PG3_finger_{side}'],new[f'PG3_finger_{side}'],row['extension_mm']) for side in ['L','R']}
    ...
    for name in printed:
        ...
        stl_checks[name] = {'brep_valid':shape.isValid(),'solid_count':len(shape.Solids()),'volume_mm3':shape.Volume(),
                            'watertight':bool(mesh.is_watertight),'mesh_components':len(mesh.split()),'stl_sha256':hashlib.sha256(stl.read_bytes()).hexdigest()}
    probes=[]
    for x,y,z in ANCHORS:
        probe=cq.Solid.makeCylinder(1.3,2,cq.Vector(x,y,z+.05))
        probes.append(abs(new['CAM5_BASE'].intersect(probe).Volume()))
    report={'cad_sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'occurrences':len(new),
            'bodies':sum(len(s.Solids()) if s.Solids() else len(s.Shells()) or len(s.Faces()) for s in new.values()),
            'preserved_occurrences':retained,'preserved_count':len(retained),'source_object_identity_preserved':True,
            'roundtrip_checks':checks,'root_mask_checks':mask_checks,'printed_parts':stl_checks,'anchor_probe_overlap_mm3':probes,
            'd405_screw_stack':r5.d405_stack(spec),'existing_urdf_matches_candidate':False,
            'roundtrip_signature_is_full_arbitrary_shape_equivalence_proof':False}
    report['pass']=all(r['pass'] for r in checks) and all(v['pass'] for v in mask_checks.values()) and max(probes)<1e-5 and all(v['brep_valid'] and v['solid_count']==1 and v['watertight'] and v['mesh_components']==1 for v in stl_checks.values())
    save('cad-export.json',report)
    if not report['pass']:
        raise AssertionError('Saved CAD contract failed; inspect cad-export.json')
    compact={r.name:r.world for r in read_step(ROOT/'outputs/optimization-250g/CAD/Robot_250g_compact_D405.step')[2]}
    for label,scene in [('compact75',compact),('low_profile30',new)]:
        render([(s,color(n)) for n,s in scene.items()],OUT/'images'/f'{label}_side.png',direction=(1,0,0),focus=(-.2,175,213),scale=90,size=(1100,900))
        render([(s,color(n)) for n,s in scene.items()],OUT/'images'/f'{label}_assembly.png',direction=(1,-.6,.5),focus=(-.2,140,170),scale=165,size=(1200,1000))
