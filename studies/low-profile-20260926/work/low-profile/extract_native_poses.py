...
def by_name(parts):
    ...
    return {name:sorted(rows,key=lambda x:(x['solids'],x['local_bounds_mm'])) for name,rows in result.items()}
...
for target in targets:
    ...
    for part_name,rows in actual.items():
        ...
        for p,ref in zip(rows,zero_by_name[part_name]):
            ...
            local_bbox_deltas.append(float(np.abs(np.array(p['local_bounds_mm'])-ref['local_bounds_mm']).max()))
    ...
    for group,parts in per_group.items():
        ...
        si=ref.copy();si[:3,3]/=1000
        instances[group]={'transform_m':si.reshape(-1).tolist(),'body_count':len(parts),
                          'max_member_rotation_element_delta':float(np.abs(matrices[:,:3,:3]-ref[:3,:3]).max()),
    ...
    for joint in joints:
        ...
        residuals[joint['name']]=float(np.linalg.norm(relative[:3,3]-(pivot-relative[:3,:3]@pivot)))
    results.append({'name':name,'step_sha256':data['sha256'],'expected_deg':target['expected_deg'],
                    'observed_from_step_deg':observed,
                    'max_angle_error_deg':max(abs(observed[n]-v) for n,v in target['expected_deg'].items()),
                    'joint_pivot_translation_residual_mm':residuals,
                    'max_local_bbox_delta_from_version_zero_mm':max(local_bbox_deltas),
                    'instances':instances})
    print(name,'angle max deg',results[-1]['max_angle_error_deg'], 'local bbox delta mm',max(local_bbox_deltas),flush=True)
max_rotation_delta=max(v['max_member_rotation_element_delta'] for c in results for v in c['instances'].values())
max_translation_delta=max(v['max_member_translation_delta_mm'] for c in results for v in c['instances'].values())
rigid_members_match=max_rotation_delta<=2e-5 and max_translation_delta/1000<=2e-5
report={'method':'XCAF occurrence placements from actual UI STEP exports, grouped by declared unique part-name membership',
        ...
        'verification_status':'RIGID_MEMBERS_PASS_FK_PENDING' if rigid_members_match else 'FAIL_RIGID_MEMBER_MOTION',
...
print('EXTRACTED',len(results),'poses and',sum(len(c['instances']) for c in results),'rigid transforms',flush=True)
