    ...
    for part_name, rows in actual.items():
        ...
        for p, ref in zip(rows, zero_by_name[part_name]):
            ...
            local_bbox_deltas.append(
                float(
                    np.abs(
                        np.array(p["local_bounds_mm"]) - ref["local_bounds_mm"]
                    ).max()
                )
            )
...
max_translation_delta = max(
    v["max_member_translation_delta_mm"]
    for c in results
    for v in c["instances"].values()
)
rigid_members_match = (
    max_rotation_delta <= 2e-5 and max_translation_delta / 1000 <= 2e-5
)
report = {
...
(OUT / "reports/native-pose-transforms.json").write_text(
...
if not rigid_members_match:
    raise SystemExit(
        "FAIL: components assigned to one rigid group do not move together"
    )
