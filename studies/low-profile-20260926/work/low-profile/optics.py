...
from study import OUT, ROOT, SIZE, cube, pose, save, screen, view_metrics
...
def main():
    ...
    groups = [('compact75',0,source,compact,pose([-.2,185,250],75)),
              ('low_profile30',row['extension_mm'],extended_arm(source,row['extension_mm']),build(candidate_spec(row)),row['pose'])]
    folder = OUT/'images/camera'
    folder.mkdir(parents=True,exist_ok=True)
    report = {'camera_body_exclusion':'Depth origins inside nominal camera body; only CAM5_D405_BODY excluded, all support/cable included',
              'hfov':84,'size':SIZE,'physical_camera_calibrated':False,'cases':[]}
    for variant, extension, arm, mount, p in groups:
        for label, angle, edge in cases():
            scene = retained_at(arm,angle) | {n:s for n,s in mount.items() if n!='CAM5_D405_BODY'}
            if edge:
            ...
            for side in ['left','right']:
                ...
                frustum = next((x for x in screen(p,extension) if x['edge']==edge and x['side']==side),None)
                passed = None if edge is None else (metric['cube']['visible_fraction']>=.9 and frustum['inside'] and frustum['min_depth_mm']>=75)
                report['cases'].append({'variant':variant,'case':label,'angle':angle,'edge':edge,'side':side,
                                        'metrics':metric,'frustum':frustum,'target_pass':passed,
                                        'image':str(image.relative_to(OUT))})
            view.close()
            save('full-optics.json',report)
            print(variant,label,[x['target_pass'] for x in report['cases'][-2:]],flush=True)
    ...
    report['selected_min_visibility'] = min(r['metrics']['cube']['visible_fraction'] for r in targets)
