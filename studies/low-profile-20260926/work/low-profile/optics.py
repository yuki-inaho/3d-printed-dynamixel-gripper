...
def main():
        ...
        for label, angle, edge in cases():
            ...
            for side in ["left", "right"]:
                ...
                frustum = next(
                    (
                        x
                        for x in screen(p, extension)
                        if x["edge"] == edge and x["side"] == side
                    ),
                    None,
                )
                passed = (
    ...
    targets = [
        r for r in report["cases"] if r["variant"] == "low_profile30" and r["edge"]
    ]
    ...
    report["selected_min_axial_depth_mm"] = min(
        r["frustum"]["min_depth_mm"] for r in targets
    )
