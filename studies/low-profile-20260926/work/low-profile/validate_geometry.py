...
def scan(parts, spec, angle):
    arm = retained_at(
        {n: s for n, s in parts.items() if not n.startswith("CAM5_")}, angle
    )
    ...
    for a, b in combinations(scene, 2):
        if (
            a not in CHANGED
            and b not in CHANGED
            and a not in camera
            and b not in camera
        ):
            continue
        tested += 1
        if box_distance(bbs[a], bbs[b]) >= 0.3:
            continue
        if a in camera and b in camera:
            ...
            contact = any(
                {a, b} == {f"PG3_finger_{side}", f"PG3_pad_{side}"}
                or {a, b} == {f"PG3_finger_{side}", f"PG3_carriage_{side}"}
                for side in ["L", "R"]
            )
    ...
    for row in new:
        ...
        if prior is None:
            ...
            out.append(
                row
                | {
                    "change": "increased_material_overlap",
                    "previous_mm3": prior.get("intersection_mm3", 0),
                }
            )
        elif (
            "gap_mm" in row
            and "gap_mm" in prior
            and row["gap_mm"] < prior["gap_mm"] - 1e-5
        ):
            out.append(
                row
                | {
                    "change": "decreased_existing_gap",
                    "previous_gap_mm": prior["gap_mm"],
                }
            )
    return out
...
def wrist(parts):
    ...
    result = {
        "axis_point_mm": point.tolist(),
        "axis_direction": axis.tolist(),
        "step_deg": 2,
        "continuous_proof": False,
        "cases": [],
    }
    for jaw in [25.0, 90.0, 135.0]:
        dynamic = retained_at(arm, jaw)
        group = mount | {n: dynamic[n] for n in CHANGED}
        row = {
            "gripper_angle_deg": jaw,
            "zero": wrist_hit(group, upstream, point, axis, 0),
            "limits": [],
            "stops": [],
        }
        for sign in [-1, 1]:
    ...
    inherited = json.loads(
        (
            REPO / "outputs/camera-mount-id5-overhead-d405-r5/reports/id4_sweep.json"
        ).read_text()
    )
    result["unchanged_arm_reference"] = inherited["arm_only"]
    result["physical_limits_deg"] = [
        max(
            inherited["arm_only"]["free_interval_deg"][0],
            *(r["limits"][0] for r in result["cases"]),
        ),
        min(
            inherited["arm_only"]["free_interval_deg"][1],
            *(r["limits"][1] for r in result["cases"]),
        ),
    ]
    result["onshape_urdf_limits_deg"] = [
        -result["physical_limits_deg"][1],
        -result["physical_limits_deg"][0],
    ]
    return result
...
def main():
    ...
    old = {
        r.name: r.world
        for r in read_step(
            ROOT / "outputs/optimization-250g/CAD/Robot_250g_compact_D405.step"
        )[2]
    }
    spec = candidate_spec(json.loads((OUT / "reports/selection.json").read_text()))
    oldspec = replace(Spec(), glass_center=(-0.2, 185, 250), pitch_deg=75)
    report = {
    ...
    for angle in range(25, 136, 5):
        ...
        record = {
            ...
            "baseline": baseline,
            "candidate": candidate,
            "new_or_worsened": newly_worse(baseline["nonpass"], candidate["nonpass"]),
        }
        ...
        print(
            ...
            angle,
            "old",
            dict(Counter(r["status"] for r in baseline["nonpass"])),
            "new",
            dict(Counter(r["status"] for r in candidate["nonpass"])),
            "worsened",
            len(record["new_or_worsened"]),
            flush=True,
        )
    report["wrist"] = wrist(parts)
    report["no_new_or_worsened_gripper_defects"] = all(
        not r["new_or_worsened"] for r in report["cases"]
    )
    report["zero_wrist_clear"] = all(
        r["zero"] is None for r in report["wrist"]["cases"]
    )
    save("geometry-validation.json", report)
    print(
        "VALIDATION",
        report["no_new_or_worsened_gripper_defects"],
        report["wrist"]["physical_limits_deg"],
        flush=True,
    )
