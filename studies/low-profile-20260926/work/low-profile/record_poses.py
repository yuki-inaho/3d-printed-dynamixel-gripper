...
report = (
    ...
    if report_path.exists()
        ...
        "cases": [
            {
                "name": "zero",
                "stored_deg": [0, 0, 0, 0, 0],
                "applied_message": True,
                "evidence": "interactive CLI zero is applied notification and onshape-pose-zero.png",
            },
            {
                "name": "small_joint1_yaw",
                "rowId": "0z/QhXG1E9KW7uLo",
                "stored_deg": [10, 0, 0, 0, 0],
                "applied_message": True,
                "evidence": "interactive pilot result and onshape-pose-small_joint1_yaw.png",
            },
        ],
    }
)
...
for row in targets:
    ...
    spec = {
        "name": name,
        "col": col,
        "deg": values[col] if col is not None else 0,
        "reuse": name,
    }
    ...
    proc = subprocess.run(
        [
            ...
            "-s=onshape-headless",
            "run-code",
            "--filename=low-profile/current-pose.js",
        ],
        cwd=ROOT / "work",
        capture_output=True,
        text=True,
    ...
    try:
        ...
        assert (
            data["name"] == name
            and data["stored_deg"] == values
            and data["applied_message"]
        )
        ...
        report.setdefault("failure_history", []).append(
            {
                ...
                "resolution": "UI retry; see timestamped WORKDOC and captured values",
            }
        )
    report_path.write_text(json.dumps(report, indent=2) + "\n")
...
print(
    "ALL",
    len(report["cases"]),
    "UI positions applied; STEP numerical verification pending",
    flush=True,
)
