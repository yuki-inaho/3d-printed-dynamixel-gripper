    ...
    comparisons.append(
        {
            ...
            "source": a,
            "native": b,
            "max_bbox_delta_mm": float(
                np.abs(np.array(a["bounds"]) - b["bounds"]).max()
            ),
...
actual = Counter(
    (p["name"], "solid" if p["solids"] else "sheet") for p in native["parts"]
)
desired = Counter(
    (p["name"], p["body_type"]) for parts in expected.values() for p in parts
)
...
result = {
    ...
    "native_bodies_by_declared_group": dict(
        Counter(mapping[p["name"]] for p in native["parts"])
    ),
...
(OUT / "reports/native-zero-source-comparison.json").write_text(
