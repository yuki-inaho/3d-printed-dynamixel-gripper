...
sys.path.insert(0, str(ROOT/'work/gripper-low-profile/scripts'))
...
for row in rows:
    shape = row.world
    source_groups[row.name].append({'bounds':bounds(shape),'volume':sum(s.Volume() for s in shape.Solids()),'area':shape.Area(),'solids':len(shape.Solids())})
for row in native['parts']:
    native_groups[row['name']].append({'bounds':row['world_bounds_mm'],'volume':row['volume_mm3'] or 0,'area':row['area_mm2'],'solids':row['solids']})

def aggregate(items):
    b=np.array([x['bounds'] for x in items])
    return {'bounds':np.r_[b[:,:3].min(axis=0),b[:,3:].max(axis=0)].tolist(),
            'volume':sum(x['volume'] for x in items),'area':sum(x['area'] for x in items),'solids':sum(x['solids'] for x in items)}

assert source_groups.keys()==native_groups.keys(), 'Part names differ'
comparisons=[]
for name in source_groups:
    a=aggregate(source_groups[name]);b=aggregate(native_groups[name])
    comparisons.append({'name':name,'source':a,'native':b,
                        'max_bbox_delta_mm':float(np.abs(np.array(a['bounds'])-b['bounds']).max()),
                        'volume_delta_mm3':b['volume']-a['volume'],'area_delta_mm2':b['area']-a['area']})
...
assert actual==desired, {'unexpected':str(actual-desired),'missing':str(desired-actual)}
result={'source':str(source),'native_step_sha256':native['sha256'],
        'names_and_body_types_match':True,'native_bodies_by_declared_group':dict(Counter(mapping[p['name']] for p in native['parts'])),
