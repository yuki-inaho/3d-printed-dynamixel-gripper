...
def extend_finger(shape, delta):
    ...
    faces = [f for f in root.Faces() if f.geomType() == 'PLANE'
             and abs(f.Center().y - CUT_Y) < 1e-6
             and f.normalAt().y > .999]
    if not faces:
        raise ValueError('No forward section face at declared cut plane')
    bridges = [cq.Solid.extrudeLinear(f.outerWire(), f.innerWires(), cq.Vector(0, delta, 0)) for f in faces]
...
def preservation(old, new, delta):
    ...
    if new.BoundingBox().ymin >= CUT_Y:
        return report | {'pass': False, 'reason': 'whole new solid is outside the preserved root region'}
    ...
    return report | {'root_difference_mm3': root_diff, 'tip_difference_mm3': tip_diff,
                     'pass': root_diff < TOL_VOLUME and tip_diff < TOL_VOLUME}


def payload_delta(delta):
    length_check(delta)
    return .25 * G * delta / 1000
