async page => {
  ...
  await d.getByRole('textbox').first().fill('low-profile-zero');
  ...
  await page.screenshot({path:'studies/low-profile-20260926/outputs/low-profile-250g/images/onshape-step-export-settings.png'});
  ...
  const path='studies/low-profile-20260926/outputs/low-profile-250g/CAD/native-poses/zero.'+ext;
  await download.saveAs(path);
  return {pose:'zero',source_url:page.url(),settings,suggested,path,error:await download.failure()};
}
