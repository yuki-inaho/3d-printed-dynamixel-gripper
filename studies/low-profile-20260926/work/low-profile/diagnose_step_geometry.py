...
files=[OUT/'CAD/Robot_250g_low_profile_D405.step', OUT/'CAD/native-poses/zero.step']
loaded=[read_step(p) for p in files]
parts=[{r.name:r.world for r in data[2]} for data in loaded]
names=['ARM_P03_shoulder_XL430','ARM_P02_yaw_carrier','ARM_M04_ref08',
       'CAM5_BASE','CAM5_D405_CARRIER','PG3_finger_L','PG3_finger_R']
results=[]
for name in names:
    ...
    for shape in [a,b]:
        ...
        vertices=[];faces=[];face_deflections=[]
        for face in shape.Faces():
            ...
            faces.extend(tuple(t.Value(j)+offset-1 for j in (1,2,3)) for t in poly.Triangles())
    ...
    result={'name':name,'source_volume_default_mm3':a.Volume(),'native_volume_default_mm3':b.Volume(),
            'source_volume_adaptive_1e10_mm3':a.Volume(1e-10),'native_volume_adaptive_1e10_mm3':b.Volume(1e-10),
...
report={'method':'Bidirectional vertices and triangle centers to opposite mesh after absolute OCCT tessellation; raw distances, no new acceptance threshold.',
        'tessellation_linear_mm':0.001,'tessellation_angular_rad':0.1,
        ...
        'parts':results}
(OUT/'reports/native-step-geometry-diagnostic.json').write_text(json.dumps(report,indent=2)+'\n')
