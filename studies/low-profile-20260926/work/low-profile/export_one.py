...
body = (
    body.replace("low-profile-zero", "low-profile-" + args.name)
    .replace("native-poses/zero.", "native-poses/" + args.name + ".")
    .replace("pose:'zero'", "pose:" + json.dumps(args.name))
    .replace("onshape-step-export-settings.png", "onshape-step-export-" + args.name + ".png")
)
if args.final_version:
    assert args.name == "zero" and args.dialog_open
    body = (
        body.replace("native-poses/zero.", "final-version.")
        .replace("low-profile-zero", "low-profile-V2-final")
        .replace("onshape-step-export-zero.png", "onshape-step-export-v2.png")
        .replace('pose:"zero"', 'pose:"version_zero"')
    )
...
proc = subprocess.run(
    [
        ...
        "-s=onshape-headless",
        "run-code",
        "--filename=low-profile/current-export.js",
    ],
    cwd=ROOT / "work",
    capture_output=True,
    text=True,
...
data = (
    ...
    if manifest.exists()
    else {"direct_api_calls": 0, "exports": []}
)
