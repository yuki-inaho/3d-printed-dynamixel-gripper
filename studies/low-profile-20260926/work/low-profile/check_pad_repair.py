...
before=json.loads((OUT/'reports/v1-pad-misassigned/native-pose-transforms.json').read_text())
...
for b,a in zip(before['cases'],after['cases'],strict=True):
    ...
    for group in ('jaw_l','jaw_r'):
        ...
        cases.append({'pose':a['name'],'group':group,
...
report={'status':'PASS','cause':'Import Update preserved stale union queries that assigned PG3_pad_L/R to opposite jaw',
        ...
        'cases':cases}
(OUT/'reports/pad-membership-repair.json').write_text(json.dumps(report,indent=2)+'\n')
print('PASS: 22 fresh jaw/pose membership checks, original defect retained as negative control')
