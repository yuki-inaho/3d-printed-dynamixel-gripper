...
再現: `rtk proxy uv run --project /home/inaho-omen/Project/3d-printed-dynamixel-gripper --no-sync python work/low-profile/research.py`。参考STLは `work/low-profile/cache/`。参考CADの再配布前には同commitのライセンスを併置する。解析JSONの数値は推測寸法ではなく今回取得したCADからの値。
