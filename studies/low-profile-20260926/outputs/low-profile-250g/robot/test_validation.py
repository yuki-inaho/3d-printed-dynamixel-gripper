...
import pytest
from validate_robot import ROOT, ValidationError, compare_native
...
def test_reject_real_v1_pad_misassignment(inputs):
    robot, _ = inputs
    bad = json.loads(
        (ROOT.parent / "reports/v1-pad-misassigned/native-pose-transforms.json").read_text()
    )
...
def test_reject_metre_millimetre_placement(inputs):
    ...
    case = next(case for case in bad["cases"] if case["name"] == "small_joint2_shoulder")
