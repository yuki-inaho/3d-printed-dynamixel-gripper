...
for j in JOINTS:
    ...
    for suffix, val in zip(["Min", "Max"], j["limits"]):
        next(p for p in f["parameters"] if p["parameterId"] == field + suffix).update(
            isNull=False,
            expression=f"{val:.12g} {unit}",
            value=val,
    ...
    f["parameters"] = [
        p
        for p in f["parameters"]
        if not p["parameterId"].startswith("limit")
        or p["parameterId"] in ["limitsEnabled", field + "Min", field + "Max"]
    ]
    r = api(
