...
for i,chunk in enumerate(chunks):
 ...
 sections.append(f'<section class="page {cls}" id="page-{i+1}"><div class="running">ONSHAPE ROBOT WORKFLOW · HUMAN GUIDE</div><div class="content">{body}</div><footer>2026-09-26　Free / Public　<span>{i+1} / {len(chunks)}</span></footer></section>')
css='''
...
page='<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Onshape ロボット作業マニュアル</title><style>'+css+'</style></head><body><nav><strong>Onshape ロボット<br>作業マニュアル</strong><div>画像をクリックすると拡大できます。</div><button onclick="window.print()">印刷 / PDFに保存</button>'+''.join(toc)+'</nav><main>'+''.join(sections)+'</main><dialog><button aria-label="閉じる">×</button><img alt="拡大画像"></dialog><script>'+js+'</script></body></html>'
