...
api = api.replace(
    ...
    "credentials={'accessKey':os.environ['ONSHAPE_ACCESS_KEY'],'secretKey':os.environ['ONSHAPE_SECRET_KEY']}",
)
api = api.replace("counter=PRIVATE_ROOT/'api-count.txt'", "counter=ROOT/'api-count.txt'")
(dest / "onshape_api.py").write_text(api)
(dest / "project.example.json").write_text(
    json.dumps(
        {
            "did": "NEW_DOCUMENT_ID",
            "wid": "NEW_WORKSPACE_ID",
            "ps": "IMPORTED_262_BODY_PARTSTUDIO_ID",
            "asm": "EMPTY_ASSEMBLY_ID",
        },
        indent=2,
    )
)
...
shutil.copytree(
    Path("skills/onshape-robot-workflow"), out / "onshape-robot-workflow", dirs_exist_ok=True
)
