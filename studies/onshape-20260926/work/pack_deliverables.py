...
for p in [
    out / "onshape-reference-version.json",
    out / "onshape-kinematic-version.json",
    out / "rebuild/reference-version.json",
    out / "rebuild/kinematic-version.json",
]:
    d = json.loads(p.read_text())
    (root / ("original-" + p.parent.name + "-" + p.name)).write_text(json.dumps(d, indent=2))
    p.write_text(
        json.dumps(
            {
                k: v
...
config["url"] = (
    "https://cad.onshape.com/documents/1f8f316ef4f488303d1c1902/v/9d614fea1354d083cde3bf56/e/282c9739d5fb8e5aa9a89f3f"
)
...
files = [
    ...
    and p.name != "delivery-check.json"
]
for p in files:
    blob = p.read_bytes()
