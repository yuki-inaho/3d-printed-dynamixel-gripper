...
old = Path("./outputs/camera-mount-id5-overhead-d405-r5/CAD/ID5_D405_mid_ASSEMBLY.step")
...
for name in sorted(a):
    ...
    rows.append({"name": name, "volume_delta_mm3": float(dv), "bounds_delta_mm": float(db)})
...
for name, details in manifest["meshes"].items():
    assert (
        hashlib.sha256((OUT / "robot/assets" / name).read_bytes()).hexdigest() == details["sha256"]
    )
for name, key in [
    ("robot.urdf", "portable_urdf_sha256"),
    ("robot.onshape-raw.urdf", "raw_urdf_sha256"),
]:
    assert hashlib.sha256((OUT / "robot" / name).read_bytes()).hexdigest() == manifest[key]
