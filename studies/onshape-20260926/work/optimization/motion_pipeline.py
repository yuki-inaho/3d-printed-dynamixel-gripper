...
def setup():
    ...
    if not (STATE / "project.json").exists():
        ...
        created = (
            matches[0] if matches else api(f"/assemblies/d/{did}/w/{wid}", "POST", {"name": NAME})
        )
    ...
    existing = {
        i["name"].split(" <")[0]: i["id"] for i in assembly(project)["rootAssembly"]["instances"]
    }
    for name in names:
        if name in existing:
            continue
...
def build():
    ...
    joint_definitions.JOINTS = json.loads((STATE / "joint-definitions.json").read_text())
...
def test_motion():
    ...
    def route(target, label):
        ...
        steps = max(
            1,
            math.ceil(max(abs(target[n] - begin[n]) for n in active) / math.radians(10)),
        )
        for step in range(1, steps + 1):
            waypoint = {n: begin[n] + (target[n] - begin[n]) * step / steps for n in active}
            values = [
                {
                    "jsonType": "Revolute",
    ...
    for label, changes in cases:
        ...
        error = max(
            abs(by_name["dof_" + name]["rotationZ"] - angle) for name, angle in expected.items()
        )
        displacement = max(
        ...
        row = {
            ...
            "achieved_rad": {name: by_name["dof_" + name]["rotationZ"] for name in active},
