...
def pose(glass, pitch=None):
    ...
    return {
        ...
        "right": right.tolist(),
        "eyes": {
            k: (g - 3.7 * d + sign * 9 * right).tolist() for k, sign in [("left", -1), ("right", 1)]
        },
    }
...
def view_metrics(view, p, side, image=None, cube_only=False):
    ...
    for label, names in {
        ...
        result[label] = {
            ...
            "visible_fraction": float((mask & np.isin(actual, codes)).sum() / mask.sum())
            if mask.any()
            else 0,
            "touches_border": bool(
...
def mass_report(mount):
    ...
    def both(origin):
        ...
        return {
            ...
            "plus_payload_reference_M_Nm": (m + moment([CENTER], [0.25], origin)).tolist(),
        }
    ...
    return {
        ...
        "com_mm": com.tolist(),
        "printed_mass_g": sum(r["mass_g"] for r in items if r["basis"].startswith("solid PLA")),
...
def validate():
            ...
            for side in ("left", "right"):
                ...
                for key, names in {
                    ...
                    if key not in met:
                        continue
                    pts = np.array(
                        [v.toTuple() for n in names for v in scene[n].tessellate(0.2, 0.3)[0]]
                    )
    ...
    side = max(
        (r for r in search_result["rendered_candidates"] if r["kind"] == "lateral_concept"),
