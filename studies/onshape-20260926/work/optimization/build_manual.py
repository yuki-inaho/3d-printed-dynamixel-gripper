...
for i, chunk in enumerate(chunks):
    ...
    sections.append(
        f'<section class="page {cls}" id="page-{i + 1}"><div class="running">D405 COMPACT MOUNT · VALIDATION GUIDE</div><div class="content">{body}</div><footer>2026-09-26　Free / Public　<span>{i + 1} / {len(chunks)}</span></footer></section>'
    )
css = """
...
page = (
    '<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>D405短縮候補・操作マニュアル</title><style>'
    + css
    + '</style></head><body><nav><strong>D405短縮候補<br>操作マニュアル</strong><div>画像をクリックすると拡大できます。</div><button onclick="window.print()">印刷 / PDFに保存</button>'
    + "".join(toc)
    + "</nav><main>"
    + "".join(sections)
