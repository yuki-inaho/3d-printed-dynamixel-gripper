...
label = sys.argv[2] if len(sys.argv) > 2 else str(angle)
...
targets = [
    {
        "jsonType": "Revolute",
        "featureId": fid,
        "rotationZ": math.radians(90 - angle),
        "ownerOccurrencePath": [],
    }
]
...
print(
    label,
    [
        (v.get("mateName"), v.get("rotationZ", v.get("translationZ")))
        for v in r["mateValues"]
        if v.get("jsonType") in ("Revolute", "Slider")
    ],
)
