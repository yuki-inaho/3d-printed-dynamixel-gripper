...
def joint_states(theta_deg=90):
    ...
    link_delta = math.atan2(
        -14 * math.sin(theta), x - 14 * math.cos(theta)
    ) - math.atan2(-14, x0)
    return {
        "joint1_yaw": 0.0,
        "joint2_shoulder": 0.0,
        "joint3_elbow": 0.0,
        "joint4_wrist": 0.0,
        "gripper_drive": -drive_delta,
        "jaw_left": (x - x0) / 1000,
        "jaw_right": -(x - x0) / 1000,
        "coupler_left": drive_delta - link_delta,
        "coupler_right": drive_delta - link_delta,
    }
...
if __name__ == "__main__":
    ...
    p.add_argument("theta", type=float, nargs="?", default=90)
    t = p.parse_args().theta
    print(
        json.dumps(
            {
                "theta_deg": t,
                "opening_mm": opening_mm(t),
                "joint_positions": joint_states(t),
            },
            indent=2,
        )
    )
