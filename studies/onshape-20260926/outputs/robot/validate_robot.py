...
def validate():
    ...
    for l in robot.findall("link"):
        ...
        meshes[l.get("name")] = {"triangles": len(mesh.faces), "path": str(m.relative_to(ROOT))}
    ...
    for theta in np.linspace(25, 135, 441):
        ...
        for j in robot.findall("joint"):
            ...
            if lim is not None:
                ...
                assert float(lim.get("lower")) - tol <= value <= float(lim.get("upper")) + tol, (
                    theta,
                    j.get("name"),
                    value,
                )
        for side in ["left", "right"]:
            err = float(
                np.linalg.norm(p[f"closing_{side}_1"][:3, 3] - p[f"closing_{side}_2"][:3, 3])
            )
            residual = max(residual, err)
        if theta in [25, 90, 135]:
            samples.append(
                {"theta_deg": float(theta), "opening_mm": opening_mm(theta), "joint_positions": q}
            )
    ...
    print(json.dumps({k: v for k, v in report.items() if k not in ["meshes", "samples"]}, indent=2))
    return robot, report
